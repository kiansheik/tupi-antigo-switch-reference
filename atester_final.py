...
# Function to get required tags and save their order
def get_required_tags():
    if os.path.exists(REQUIRED_TAGS_FILE):
        with open(REQUIRED_TAGS_FILE, "r") as f:
            return json.load(f)
...
filtered_data = [entry for entry in data if re.search(pattern, entry.get('d', ''))]
print(f"Found {len(filtered_data)} matching entries.")
...
while i < len(filtered_data):
    ...
    # Prompt user to annotate entry with tags in order
    prompt_for_tags_in_order(entry, required_tags)

    # Get user action: done, skip, or back
    action = input("Enter 'done' to save, 'skip' to skip, 'back' to go to the previous entry: ").strip().lower()
    if action == "done":
        # Save the annotated entry to the annotation map
        annotation_map[entry['d']] = entry
        i += 1  # Move to the next entry
